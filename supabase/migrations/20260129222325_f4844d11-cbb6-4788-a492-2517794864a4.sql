CREATE OR REPLACE FUNCTION public.import_mission_pack(_pack_json jsonb, _actor_user_id uuid, _mode text DEFAULT 'draft'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      GET STACKED DIAGNOSTICS 
        ...
        _err_message = MESSAGE_TEXT,
        _err_detail = PG_EXCEPTION_DETAIL,
        _err_hint = PG_EXCEPTION_HINT,
        _err_context = PG_EXCEPTION_CONTEXT;
      
      _errors := _errors || jsonb_build_object(
        'index', _idx,
        'reason', _err_message,
        'sqlstate', _err_sqlstate,
        'detail', COALESCE(_err_detail, ''),
        'hint', COALESCE(_err_hint, ''),
        'context', COALESCE(_err_context, ''),
        'item', _mission
      );
    END;
