-- Fix generate_conversation_mission: avoid invalid enum values in crm_contatos.status filter
...
BEGIN
  ...
  -- Get contacts that need follow-up (FIX: use valid crm_contato_status values)
  SELECT array_agg(id) INTO v_contact_ids
  FROM (
    SELECT id FROM public.crm_contatos
    WHERE criado_por = v_user_id
      AND status IN ('novo', 'contatar', 'em_conversa')
