-- Fix import_mission_pack to use only valid mission_status enum values
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Determine status text with proper priority: mode > item.status > defaults.status > 'rascunho'
      ...
      -- Map status text to valid enum values only:
      -- Valid: rascunho, publicada, em_andamento, enviada, validada, reprovada, concluida
      v_status := CASE v_status_text
        ...
        WHEN 'em_andamento' THEN 'em_andamento'::mission_status
        WHEN 'enviada' THEN 'enviada'::mission_status
        WHEN 'validada' THEN 'validada'::mission_status
        WHEN 'reprovada' THEN 'reprovada'::mission_status
        WHEN 'concluida' THEN 'concluida'::mission_status
