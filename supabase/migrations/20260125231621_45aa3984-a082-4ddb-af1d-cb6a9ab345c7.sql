-- Add first action tracking columns to profiles
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS first_action_at TIMESTAMPTZ NULL,
ADD COLUMN IF NOT EXISTS first_action_kind TEXT NULL;

COMMENT ON COLUMN public.profiles.first_action_at IS 'Timestamp when user completed their first action';
COMMENT ON COLUMN public.profiles.first_action_kind IS 'Type of first action: rua, conversa, crm, followup';

-- Update growth_events constraint to include first_action event type
...
-- Update log_growth_event RPC to include first_action
